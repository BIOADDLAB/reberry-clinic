            ...
            >
                {list.map((item) => (
                    <SwiperSlide key={item.slug} className="!w-auto">
                        <article className="flex h-full w-[240px] shrink-0 flex-col overflow-hidden rounded-[15px] bg-cream ring-1 ring-cocoa/10 md:w-[262px]">
                            <div className="relative w-full aspect-[262/253]">
                                <Image
                                    src={item.image}
                                    alt={item.name}
                                    fill
                            ...
                            <div className="flex flex-1 flex-col bg-sand px-4 pb-8 pt-6.5">
                                ...
                                >
                                    ...
                                    <span className="relative flex h-1.75 w-1.75 items-center justify-center rounded-full bg-white/25 animate-pulse-slow">
                                        <span className="relative block h-0.5 w-0.5 rounded-full bg-white"></span>
                                    </span>
                                </Link>
                                <h3 className="ml-2 mt-6 font-bold text-lead">{item.name}</h3>
                                <ul className="mt-2 flex-1 space-y-0.2 font-medium tracking-tighter text-small">
                                    {item.desc.map((line) => (
                                        <li key={line}>- {line}</li>
                                    ))}
                                </ul>
